...
const calculateTotalDuration = (parkedDate, parkedTime, exitDate, exitTime) => {
  ...
  try {
    ...
    const endTime = new Date(parseInt(endYear), parseInt(endMonth) - 1, parseInt(endDay), endHours, endMinutes)
  ...
  const handleConfirmDelete = async () => {
    if (isBulkDeleting) {
      ...
      try {
        ...
        setData(prevData => prevData.filter(booking => !idsToDelete.includes(booking._id)))
        setFilteredData(prevData => prevData.filter(booking => !idsToDelete.includes(booking._id)))
        setRowSelection({})
      } catch (error) {
      ...
      try {
        ...
        if (success) {
          setData(prevData => prevData.filter(booking => booking._id !== bookingToDelete))
          setFilteredData(prevData => prevData.filter(booking => booking._id !== bookingToDelete))
        }
      } catch (error) {
  ...
  const exportToExcel = () => {
    ...
    dataToExport.forEach((row, index) => {
      ...
      if (row.status?.toLowerCase() === 'completed') {
        duration = calculateTotalDuration(row.parkedDate, row.parkedTime, row.exitvehicledate, row.exitvehicletime)
      }
    ...
    () => [
      ...
      columnHelper.accessor('vendorName', {
        ...
        cell: ({ row }) => {
          ...
          return <Typography>{vendor.vendorName || row.original.vendorName || 'N/A'}</Typography>
        }
      }),
      ...
      columnHelper.accessor('sts', {
        ...
        cell: ({ row }) => {
          ...
          return (
            <Typography sx={{ color: chipData.color, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 1 }}>
      ...
      columnHelper.accessor('payableTime', {
        ...
        cell: ({ row }) => {
          ...
          return <Typography>{row.original.hour || 'N/A'}</Typography>
        }
      }),
      columnHelper.accessor('duration', {
        ...
        cell: ({ row }) => {
          ...
          if (status === 'completed') {
            ...
            return <Typography sx={{ fontWeight: 500, color: '#666CFF' }}>{duration}</Typography>
          }
      ...
      columnHelper.accessor('handlingFee', {
        header: 'Handling Fee',
        cell: ({ row }) => <Typography>{row.original.handlingfee ? `₹${row.original.handlingfee}` : '₹0'}</Typography>
      }),
      columnHelper.accessor('gst', {
        header: 'GST',
        cell: ({ row }) => <Typography>{row.original.gstamout ? `₹${row.original.gstamout}` : '₹0'}</Typography>
      }),
    ...
    <Card>
      ...
      <CardContent className='flex justify-between max-sm:flex-col sm:items-center gap-4'>
        ...
        <div className='flex gap-2 items-center'>
          ...
          <Button variant='outlined' startIcon={<Download />} onClick={handleDownloadClick}>
            Download
          </Button>
