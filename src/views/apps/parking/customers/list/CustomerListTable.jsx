...
const CustomerListTable = () => {
  ...
  const handleAddVehicleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(`https://api.parkmywheels.com/add-vehicle?id=${selectedUser.uuid}`, formData, {
        headers: {
  ...
  const handleDeleteVehicleConfirm = async () => {
    ...
    try {
      const response = await axios.delete(`https://api.parkmywheels.com/deletevehicle?vehicleId=${vehicleToDelete._id}`)
  ...
  const handleTabChange = (event, newValue) => {
    ...
    if (newValue === 3 && vendorSpaces.length > 0) {
      // Amenities tab
      // Will be triggered after vendor selection
    }

    if (newValue === 4 && vendorSpaces.length > 0) {
      // Services tab
      // Will be triggered after vendor selection
    }

    if (newValue === 5 && vendorSpaces.length > 0) {
      // Services tab
      // Will be triggered after vendor selection
    }

    if (newValue === 6 && vendorSpaces.length > 0) {
      // Services tab
      // Will be triggered after vendor selection
    }

    if (newValue === 7 && vendorSpaces.length > 0) {
      // Services tab
      // Will be triggered after vendor selection
    }

    if (newValue === 8 && vendorSpaces.length > 0) {
      // Services tab
      // Will be triggered after vendor selection
    }
  ...
  const handleRescheduleSubmit = async () => {
    ...
    try {
      const response = await axios.put(`https://api.parkmywheels.com/updatebookingbyid/${selectedBooking._id}`, {
  ...
  const handleCancelBooking = async booking => {
    ...
    try {
      const response = await axios.put(`https://api.parkmywheels.com/updatebookingbyid/${booking._id}`, {
        status: 'Cancelled'
      })
                    ...
                    <TableRow key={key} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                      <TableCell>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          {charge.category === 'Bike' && (
                            <i className='ri-motorbike-fill' style={{ color: '#72e128' }} />
                          )}
  ...
  return (
    <Card>
      <CardContent>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            ...
            <Menu anchorEl={anchorEl} open={isMenuOpen} onClose={handleExportClose}>
          ...
          <DialogContent>
            <Box sx={{ mb: 2 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Typography variant='h5'>Customer ID #{selectedUser.id}</Typography>
                <Button
                  variant='contained'
                  color='primary'
            ...
            <Box sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
              ...
              >
                <Tab
                  icon={
                    <Avatar sx={{ bgcolor: '#329a73', width: 24, height: 24 }}>
                      <i className='ri-user-line' style={{ fontSize: '14px', color: 'white' }} />
                    </Avatar>
                  }
                  iconPosition='start'
                  label='Profile'
                ...
                <Tab
                  ...
                  iconPosition='start'
                  label='Myspace Services & Pricing'
                />
                      ...
                      <TableBody>
                        {vendorSpaces.map(space => (
                          <TableRow key={space._id}>
                            <TableCell>
                              <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                                {space.image && <Avatar src={space.image} sx={{ width: 40, height: 40 }} />}
                                <Typography>{space.vendorName}</Typography>
                              </Box>
                            </TableCell>
                            <TableCell>
                            ...
                            <TableCell>
                              ...
                              <Typography>{space.subscription === 'true' ? 'Active' : 'Inactive'}</Typography>
                              {space.subscriptionenddate && (
                                <Typography variant='body2' color='text.secondary'>
                                  Ends: {new Date(space.subscriptionenddate).toLocaleDateString()}
                                </Typography>
                            ...
                            <TableCell>
                              {space.parkingEntries?.map((entry, index) => (
                                <Chip key={index} label={`${entry.type}: ${entry.count}`} sx={{ mr: 1, mb: 1 }} />
                              ))}
                            </TableCell>
            ...
            {currentTab === 11 && (
              <Box sx={{ p: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                  ...
                  <Button variant='contained' onClick={handleAddVehicleClick} sx={{ backgroundColor: '#329a73' }}>
                    Add Vehicle
                  </Button>
                </Box>
                {vehiclesLoading ? (
                      ...
                      <TableBody>
                        {vehicles.map(vehicle => (
                          <TableRow key={vehicle._id}>
                            ...
                            <TableCell>
                              <IconButton onClick={() => handleDeleteVehicleClick(vehicle)} color='error'>
                                <DeleteIcon />
                              </IconButton>
                            </TableCell>
                          </TableRow>
        ...
        <DialogContent sx={{ mt: 2 }}>
          ...
          {userToDelete && (
            <Box sx={{ mt: 2, p: 2, bgcolor: '#f5f5f5', borderRadius: 1 }}>
              ...
              <Typography>
                <strong>Email:</strong> {userToDelete.userEmail || 'N/A'}
              </Typography>
              <Typography>
                <strong>Mobile:</strong> {userToDelete.userMobile || 'N/A'}
              </Typography>
            </Box>
          )}
      ...
      {/* Add Vehicle Dialog */}
      <Dialog open={addVehicleDialogOpen} onClose={handleAddVehicleClose} maxWidth='sm' fullWidth>
        ...
        <DialogContent>
          <Box sx={{ mt: 2 }}>
            ...
            <Box sx={{ mt: 2 }}>
              ...
              <input type='file' accept='image/*' onChange={handleVehicleImageChange} required />
            </Box>
          </Box>
        </DialogContent>
        <DialogActions>
      ...
      <Dialog open={deleteVehicleDialogOpen} onClose={handleDeleteVehicleCancel} maxWidth='sm' fullWidth>
        <DialogTitle sx={{ bgcolor: '#ffebee', color: '#d32f2f', py: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <DeleteIcon color='error' />
        ...
        <DialogContent sx={{ mt: 2 }}>
          ...
          {vehicleToDelete && (
            <Box sx={{ mt: 2, p: 2, bgcolor: '#f5f5f5', borderRadius: 1 }}>
              <Typography>
                <strong>Vehicle No:</strong> {vehicleToDelete.vehicleNo}
              </Typography>
              <Typography>
                <strong>Make/Model:</strong> {vehicleToDelete.make} {vehicleToDelete.model}
              </Typography>
            </Box>
          )}
          <Typography variant='body2' color='error' sx={{ mt: 2 }}>
        ...
        <DialogActions sx={{ px: 3, pb: 3 }}>
          <Button onClick={handleDeleteVehicleCancel} variant='outlined' disabled={deleteVehicleLoading}>
            Cancel
          </Button>
          <Button
      ...
      >
        <Alert onClose={handleSnackbarClose} severity='success' sx={{ width: '100%', boxShadow: 3 }}>
