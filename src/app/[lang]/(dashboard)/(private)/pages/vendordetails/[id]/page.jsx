  ...
  const handleUpdateVendorFee = async () => {
    try {
      ...
      if (response.status === 200) {
        setSnackbar({
          open: true,
          message: 'Vendor booking fee updated successfully!',
          severity: 'success'
        })
      }
    } catch (error) {
      ...
      setSnackbar({
        open: true,
        message: 'Failed to update vendor booking fee',
        severity: 'error'
      })
    } finally {
      setIsUpdatingVendorFee(false)
    }
  }

  const updateValidityDate = async () => {
    try {
      ...
      <CardContent>
        ...
        <Grid container spacing={3} sx={{ mt: 1 }}>
          <Grid item xs={12} sx={{ display: 'flex', justifyContent: 'center', mb: 2 }}>
            {imagePreview && (
              <img src={imagePreview} alt='Vendor Image' style={{ width: 100, height: 100, borderRadius: '50%' }} />
            )}
          </Grid>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <TextField
                    fullWidth
                    label='Vendor Booking Platform Fee (%)'
                    value={platformFeeVendor}
                    onChange={e => setPlatformFeeVendor(e.target.value)}
                    type='number'
                    inputProps={{ min: 0, max: 100, step: 0.1 }}
                    placeholder='Enter platform fee percentage'
                  />
                  <Button
                    variant='contained'
                    onClick={handleUpdateVendorFee}
                    disabled={isUpdatingVendorFee}
                    sx={{ minWidth: 120 }}
                  >
