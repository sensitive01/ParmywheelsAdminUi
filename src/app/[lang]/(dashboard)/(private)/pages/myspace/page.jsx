...
const SpaceDetailModal = ({ open, handleClose, vendorId }) => {
  ...
  const renderSubscription = () => {
    ...
    const endDate = spaceData.subscriptionenddate ? new Date(spaceData.subscriptionenddate).toLocaleDateString() : 'N/A'
              ...
              <Box>
                ...
                <Box sx={{ display: 'flex', gap: 1, mt: 0.5 }}>
                  <Chip
                    ...
                    size='small'
                    color={
                      spaceData.status === 'approved'
                        ? 'success'
                        : spaceData.status === 'rejected'
                          ? 'error'
                          : spaceData.status === 'suspended'
                            ? 'default'
                            : 'warning'
                    }
                  />
                  <Chip label={`ID: ${spaceData.vendorId}`} variant='outlined' size='small' />
                </Box>
              </Box>
            </Box>
    ...
    () => [
      ...
      columnHelper.accessor('vendorName', {
        ...
        cell: ({ row }) => {
          ...
          const imgSrc =
            space.image || 'https://demos.pixinvent.com/materialize-nextjs-admin-template/demo-1/images/avatars/1.png'

          return (
            <div className='flex items-center gap-3'>
              {space.image ? (
                <img src={imgSrc} alt='Space Image' className='w-8 h-8 rounded-full' />
              ) : (
            ...
            <div className='flex flex-col gap-1'>
              {parkingEntries.map((entry, index) => (
                <div key={index} className='flex items-center gap-2'>
                  <i
                    className={entry.type?.toLowerCase() === 'bikes' ? 'ri-motorbike-fill' : 'ri-car-fill'}
                    style={{ fontSize: '14px', color: entry.type?.toLowerCase() === 'bikes' ? '#72e128' : '#ff4d49' }}
                  ></i>
          ...
          return (
            <Box sx={{ display: 'flex', gap: 1 }}>
              ...
              <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
                ...
                <DialogContent>
                  <Typography>
                    Are you sure you want to delete space <strong>{row.original.vendorName}</strong> (ID:{' '}
    ...
    <Card>
      ...
      <CardContent className='flex justify-between items-center gap-4 flex-wrap'>
        ...
        <div className='flex items-center gap-4'>
          ...
          <Menu anchorEl={anchorEl} open={openMenu} onClose={handleExportClose}>
            <MenuItem
              onClick={() => {
                exportToExcel()
                handleExportClose()
          ...
          <Button variant='contained' startIcon={<AddIcon />} onClick={() => setUserModalOpen(true)}>
            Add My Space
          </Button>
        </div>
      </CardContent>
      <UserListModal open={userModalOpen} onClose={() => setUserModalOpen(false)} onUserSelect={handleUserSelect} />
      ...
      {/* Space Detail Modal */}
      <SpaceDetailModal open={modalOpen} handleClose={handleCloseModal} vendorId={selectedVendorId} />
    </Card>
  )
}
